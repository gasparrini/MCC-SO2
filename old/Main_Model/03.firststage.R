...
for(i in seq(length(dlist))) {
  ...
  cbso2lag1 <- crossbasis(data$so2, lag=3, argvar=list("lin"))
  
  cbso2lag2 <- crossbasis(data$so2, lag=3, argvar=list("lin"),arglag=list(fun="integer"))
  ...
  m1 <- glm(y~cbso2lag1+cbtemp+dow+bs(date,df=round(dfseas*length(date)/365.25)),
    data, family=quasipoisson)
  sub <- !seq(nrow(data)) %in% m1$na.action
  m2 <- glm(y~cbso2lag2+cbtemp+dow+bs(date,df=round(dfseas*length(date)/365.25)),
  ...
  cp1 <- crossreduce(cbso2lag1, m1, cen=0, at=10)
  est1[i,] <- c(cp1$fit, cp1$se^2)
  
  # FOR MODEL m2 WE HAVE TO SUM THE EFFECT OVER LAGS 0-3
  cp2 <- crossreduce(cbso2lag2, m2, cen=0, at=10)
