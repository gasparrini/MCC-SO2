...
est2 <- array(NA,dim=c(nrow(cities),2),
...
for(i in seq(length(dlist))) {
  ...
  # KEEP CITIES WITH AT LEAST 30% OF PM10 DATA
  if(sum(is.na(runMean(data$so2, 0:3)))/(subrange[2]-subrange[1]+1)>=0.7)  {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  # KEEP CITIES WITH AT LEAST 30% OF PM10 DATA
  if(as.character(cities$city[i])=="cddr.spa9014")  {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  so2knots <- quantile(data$so2,c(33,66)/100,na.rm=T)
  cbso2bs <- crossbasis(data$so2, lag=3, argvar=list(fun="bs",degree=2,knots=so2knots))
  
  # THIS SPECIFICATION MODEL THE so2 WITH UNCONTSRAINED EFFECT OVER LAGS 0-2
  cbso2lin <- crossbasis(data$so2, lag=3, argvar=list("lin"))
  ...
  m1 <- glm(y~cbso2bs+cbtemp+dow+bs(date,df=round(dfseas*length(date)/365.25)),
    data, family=quasipoisson)
  sub <- !seq(nrow(data)) %in% m1$na.action
  m2 <- glm(y~cbso2lin+cbtemp+dow+bs(date,df=round(dfseas*length(date)/365.25)),
  ...
  cp1 <- crossreduce(cbso2bs, m1, type="lag",cen=5, value=0)
  est1[i,] <- cp1$coef
  vcov1[[i]]<-cp1$vcov 
  
  cp2 <- crosspred(cbso2lin, m2, at=10, cen=0)
