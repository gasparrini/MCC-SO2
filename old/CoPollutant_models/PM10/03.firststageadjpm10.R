...
for(i in seq(length(dlist))) {
  ...
  # KEEP CITIES WITH PM10 DATA
  if("pm10"%in%names(data)==FALSE) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  # KEEP CITIES WITH PM10 DATA
  if(sum(is.na(data$pm10))==nrow(data)) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  if(sum(is.na(runMean(data$pm10, 0:3)))/(subrange[2]-subrange[1]+1)>=0.7)  {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  cbpm10 <- crossbasis(data$pm10, lag=3, argvar=list("lin"))
  ...
  m2 <- glm(y~cbso2lag+cbtemp+cbpm10+dow+bs(date,df=round(dfseas*length(date)/365.25)),
