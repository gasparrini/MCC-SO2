...
for(i in seq(length(dlist))) {
  ...
  # KEEP CITIES WITH O3 DATA
  if(sum(is.na(data$o3))==nrow(data)) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  if(sum(is.na(runMean(data$o3, 0:3)))/(subrange[2]-subrange[1]+1)>=0.7)  {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  cbo3 <- crossbasis(data$o3, lag=3, argvar=list("lin"))
  ...
  m2 <- glm(y~cbso2lag+cbtemp+cbo3+dow+bs(date,df=round(dfseas*length(date)/365.25)),
