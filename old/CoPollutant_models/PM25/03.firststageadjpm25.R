...
for(i in seq(length(dlist))) {
  ...
  # KEEP CITIES WITH PM25 DATA
  if("pm25"%in%names(data)==FALSE) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  # KEEP CITIES WITH PM25 DATA
  if(sum(is.na(data$pm25))==nrow(data)) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  
  # KEEP CITIES WITH AT LEAST 40% OF PM25 DATA
  if(sum(is.na(runMean(data$pm25, 0:3)))/(subrange[2]-subrange[1]+1)>=0.65) {
    converg[i] <- FALSE
    cat(sum(is.na(runMean(data$pm25, 0:3)))/(subrange[2]-subrange[1]+1))
    cat(" - non-convergence")
    next
  }
  ...
  cbpm25 <- crossbasis(data$pm25, lag=3, argvar=list("lin"))
  ...
  m2 <- glm(y~cbso2lag+cbtemp+cbpm25+dow+bs(date,df=round(dfseas*length(date)/365.25)),
