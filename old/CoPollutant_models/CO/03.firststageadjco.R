...
for(i in seq(length(dlist))) {
  ...
  # KEEP CITIES WITH O3 DATA
  if(sum(is.na(data$co))==nrow(data)) {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  if(sum(is.na(runMean(data$co, 0:3)))/(subrange[2]-subrange[1]+1)>=0.7)  {
    converg[i] <- FALSE
    cat(" - non-convergence")
    next
  }
  ...
  cbco <- crossbasis(data$co, lag=3, argvar=list("lin"))
  ...
  m2 <- glm(y~cbso2lag+cbtemp+cbco+dow+bs(date,df=round(dfseas*length(date)/365.25)),
