...
meta3 <- mixmeta(y3, S3, random=~1|country/city, data=cities, 
  control=list(showiter=T))

summary(meta3)

# THESE COMMANDS DERIVE THE RER OF THE POOLED EFFECT
beta3<-meta3$coefficients
se3<-sqrt(meta3$vcov)
(betaoverall3<-(exp(beta3)-1)*100)
(betaoverallinf3<-(exp((beta3-1.96*se3))-1)*100)
(betaoverallsup3<-(exp((beta3+1.96*se3))-1)*100)

# STORE BLUPS AT LOCATION AND COUNTRY LEVEL
blupcity3 <- blup(meta3,vcov=T,format="list")
blupcountry3 <- unique(blup(meta3,vcov=T,level=1,format="list"))

resultsbycountry3<-matrix(NA, nrow=length(countries), ncol=4)
resultsbycountry3<-as.data.frame(resultsbycountry3)

for (i in 1:length(countries)) {
  beta3<-blupcountry3[[i]]$blup
  se3<-sqrt(blupcountry3[[i]]$vcov)
  betaoverall3<-(exp(beta3)-1)*100
  betaoverallinf3<-(exp((beta3-1.96*se2))-1)*100
  betaoverallsup3<-(exp((beta3+1.96*se2))-1)*100
  resultsbycountry3[i,1]<-as.character(countries[i])
  resultsbycountry3[i,2]<-betaoverall3
  resultsbycountry3[i,3]<-betaoverallinf3
  resultsbycountry3[i,4]<-betaoverallsup3
}

colnames(resultsbycountry3)<-c("Country","ERR","ERRInf","ERRSup")

print(resultsbycountry3)

# PREDICTION
lcp3.all <- crosspred(so2lin,coef=coef(meta3),vcov=vcov(meta3)
                      ,model.link="log",by=0.1,cen=50)
