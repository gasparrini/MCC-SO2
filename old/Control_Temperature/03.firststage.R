...
est1 <- est2 <- est3<-array(NA,dim=c(nrow(cities),2),
  dimnames=list(cities$city,c("logrr","vcov")))

# MATRIX TO STORE THE Q-AIC
qaic <- array(NA,dim=c(nrow(cities),3),dimnames=list(cities$city,c("m1","m2","m3")))
...
for(i in seq(length(dlist))) {
  ...
  cbtemp1 <- crossbasis(data$tmean, lag=3, 
    argvar=list(fun="bs",degree=2,knots=tempknots))
  
  cbtemp2 <- crossbasis(data$tmean, lag=3, 
    argvar=list(fun="bs",degree=2,knots=tempknots),arglag=list(fun="strata",breaks= c(1)))
  
  
  cbtemp3 <- crossbasis(data$tmean, lag=3, 
    argvar=list(fun="bs",degree=2,knots=tempknots),arglag=list(fun="integer"))
  
  # RUN THE MODELS (NB: USE THE SAME OBS)
  m1 <- glm(y~cbso2lag+cbtemp1+dow+bs(date,df=round(dfseas*length(date)/365.25)),
    data, family=quasipoisson)
  sub <- !seq(nrow(data)) %in% m1$na.action
  m2 <- glm(y~cbso2lag+cbtemp2+dow+bs(date,df=round(dfseas*length(date)/365.25)),
    data, family=quasipoisson, subset=sub)
  
  m3 <- glm(y~cbso2lag+cbtemp3+dow+bs(date,df=round(dfseas*length(date)/365.25)),
    data, family=quasipoisson, subset=sub)
  ...
  cp1 <- crossreduce(cbso2lag, m1, cen=0, at=10)
  est1[i,] <- c(cp1$fit, cp1$se^2)
  
  # FOR MODEL m2 WE HAVE TO SUM THE EFFECT OVER LAGS 0-3
  cp2 <- crossreduce(cbso2lag, m2, cen=0, at=10)
  est2[i,] <- c(cp2$fit, cp2$se^2)
  
  cp3 <- crossreduce(cbso2lag, m3, cen=0, at=10)
  est3[i,] <- c(cp3$fit, cp3$se^2)
  
  # HERE THE QAIC IS STORED FOR EACH MODEL
  qaic[i,] <- c(QAIC(m1), QAIC(m2), QAIC(m3))
}
proc.time()[3]-time
